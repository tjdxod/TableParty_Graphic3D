...
namespace BNG {
    ...
    public class AutoPoser : MonoBehaviour {
        ...
        public FingerJoint[]  GetJointsCopy(FingerJoint[] ToClone) {
            FingerJoint[] joints = new FingerJoint[ToClone.Length];

            for (int x = 0; x < ToClone.Length; x++) {
                joints[x] = GetJointCopy(ToClone[x]);
            }
        ...
        public virtual bool LoopThroughJoints(Transform[] fromJoints, FingerJoint[] toJoints, Vector3 tipPosition, float tipRadius) {

            // Not a complete set
            if(fromJoints == null || toJoints == null || toJoints.Length == 0) {
                return false;
            }

            int fromLength = fromJoints.Length;
            int toLength = toJoints.Length;
            ...
            for (int x = 0; x < fromLength; x++) {
                ...
                if (isFingerTip) {
                    ...
                    var length = Physics.OverlapSphereNonAlloc(tipPosition, tipRadius, hitColliders, CollisionLayerMask, QueryTriggerInteraction.Ignore);

                    if (length > 0)
                    {
                        for(var i = 0; i < length; i++)
                        {
                            if(IsValidCollision(hitColliders[i])) {
                                return true;
                            }
                        }
                    }
