...
{
    /// <summary>
    /// Open XR 사용 시 실행되는 Input Handler 클래스
    /// </summary>
    public class PXRInputHandlerOpenXR : PXRInputHandlerBase
    {
        ...
        protected override void CheckButtonState(Buttons button, ref ButtonState buttonState)
        {
            ...
            switch (button)
            {
                case Buttons.Trigger:
                    ...
                    var pressValue = triggerReference.action.ReadValue<float>();
                    
                    buttonState.isTouch = touchValue > touchedThreshold;
                    
                    if (!buttonState.isTouch)
                    {
                        buttonState.isTouch = pressValue > 0f;
                    }

                    buttonState.value = pressValue;
                ...
                case Buttons.PrimaryAxis:
                    ...
                    if (!buttonState.isTouch)
                    {
                        var axis = GetAxisValue(ControllerAxis.Primary);
                        buttonState.isTouch = axis is not {x: 0, y: 0};
                    }
                    
                    clickValue = primaryAxisClickReference.action.ReadValue<float>();
                    buttonState.value = clickValue > clickThreshold ? 1f : 0f;
                    
                    SetButtonState(ref buttonState, clickValue > clickThreshold);
                    buttonStates[button] = buttonState;
                    break;
