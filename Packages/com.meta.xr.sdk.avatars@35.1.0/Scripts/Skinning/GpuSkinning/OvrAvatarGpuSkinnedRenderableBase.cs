...
namespace Oculus.Skinning.GpuSkinning
{
    ...
    public abstract class OvrAvatarGpuSkinnedRenderableBase : OvrAvatarSkinnedRenderable
    {
        ...
        {
            using var livePerfMarker = OvrAvatarXLivePerf_Marker(ovrAvatarXLivePerf_UnitySectionID.Skinned_Gpu_AnimationFrameUpdate);
            ...
            if (updateJoints)
            {
                jointsUpdated = UpdateJointMatrices(entityId, primitiveInstanceId);
            }
        ...
        protected internal override void ApplyMeshPrimitive(OvrAvatarPrimitive primitive)
        {
            ...
            try
            {
                ...
                _numJoints = (int)primitive.JointCount;
        ...
        {
            _canUpdateMorphs = _gpuCombiner != null && _handleInCombiner.IsValid();
        }

        private void CacheJointUpdateAbility()
        {
            _canUpdateJoints = _skinner is { HasJoints: true } && _handleInSkinner.IsValid();
        }
            ...
            CAPI.ovrAvatar2PrimitiveRenderInstanceID primitiveInstanceId)
        {
            if (_canUpdateMorphs)
            {
                // NOTE: _canUpdateMorphs check above should handle null/validity checks
                var bufferPtr = _gpuCombiner.GetMorphBuffer(_handleInCombiner);

                var success = FetchMorphTargetWeights(entityId, primitiveInstanceId, bufferPtr, OvrComputeBufferPool.WeightsSize, logScope);
            ...
            CAPI.ovrAvatar2PrimitiveRenderInstanceID primitiveInstanceId)
        {
            ...
            UInt32 bufferSize = (UInt32)(OvrComputeBufferPool.JointDataSize * _numJoints);

            IntPtr transformsSlice = _skinner.GetJointTransformMatricesArray(_handleInSkinner);

            if (transformsSlice == IntPtr.Zero)
            {
                return false;
            }

            const bool INTERLEAVE_MATRICES = true;
            var success = FetchJointMatrices(
                entityId,
                primitiveInstanceId,
                transformsSlice,
                bufferSize,
