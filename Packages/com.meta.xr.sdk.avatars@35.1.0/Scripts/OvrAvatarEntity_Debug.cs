...
namespace Oculus.Avatar2
{
    public partial class OvrAvatarEntity
    {
        ...
        // TODO: T191054744 (remove)
        [Obsolete("Use OvrAvatar2Entity_LoadUri instead.")]
        protected bool StressReloading_LoadUri(string uri)
        {
            var result = CAPI.OvrAvatar2Entity_LoadUri(entityId, uri, out var loadRequestId);
            if (!result.EnsureSuccessOnlyWarning(msgContext: "OvrAvatar2Entity_LoadUri"))
            {
                return false;
            }
        ...
        private void EditorDebugDrawPose(in CAPI.ovrAvatar2Pose pose)
        {
            ...
            for (int i = 0; i < count; ++i)
            {
                unsafe
                {
                    ...
                    (*jointTransform).DebugDrawInEditor(0.03f);
        ...
        private void EditorDebugDrawSkelHierarchy()
        {
            ...
            for (int i = 0; i < SkeletonJointCount; ++i)
            {
                ...
                joint.transform.DebugDrawInEditor(0.03f);
