...
avatar_FragOptions getFragOptions(float materialTypeChannel)
{
    ...
    if (true)
    {
        options.enableSkin = material_UseSkin(materialTypeChannel);
        options.enableEyeGlint = material_UseEyeGlint(materialTypeChannel);
        options.enableHeadHair = material_UseHeadHair(materialTypeChannel);
        options.enableFacialHair =  material_UseFacialHair(materialTypeChannel);
...
void frag_Fragment_main()
{
    ...
    o_1.a_specular *= ((specularFactor * i.material.ambient_specular_factor));
    if (i.options.enableAnyHair)
    {
    ...
    // * i.material.occlusion?
    if(isEyeGlintLightUsed > 0 || i.options.enableAnyHair) {
        metallic = 1.0;
    }
    else {
        metallic = 1.0 - i.material.metallic;
    }
    float3 diffuseContribution = ( _6005.a_diffuse) * i.material.base_color * i.material.occlusion;

    float3 specularContribution = _5982.a_specular + _5982.p_specular;
    float3 finalColor_1 = specularContribution + diffuseContribution + rimLight + _6005.subSurfaceColor + glintSpecular + glintAmbient;
    o_1.preToneMap = finalColor_1;
    o_1.color = float4(finalColor_1, o_1.color.w);
    o_1.color.w = 1.0f;
