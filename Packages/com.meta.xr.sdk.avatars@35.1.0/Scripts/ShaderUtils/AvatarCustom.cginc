...
void OvrPopulateVertexDataFromExternalTexture(bool hasTangents, bool applyOffsetAndBias, inout OvrVertexData vertData) {
  ...
  void OvrPopulateVertexDataFromExternalBuffers(bool hasTangents, bool interpolateAttributes, inout OvrVertexData vertData) {
    ...
    if (interpolateAttributes) {
      ...
      const float3 latestNorm = OvrGetFrenetEntryFromExternalBuffer(normEntryIndex).xyz;
      const float3 prevNorm = OvrGetFrenetEntryFromExternalBuffer(startIndexOfFrenetEntries + _OvrAttributeOutputPrevAnimFrameEntryOffset).xyz;
      ...
      if (hasTangents) {
        ...
        vertData.tangent = float4(lerp(prevTan, latestTan.xyz, lerpValue), latestTan.w);
      }
    } else {
      vertData.position = float4(OvrGetPositionEntryFromExternalBuffer(posEntryIndex), 1.0);
      vertData.normal = OvrGetFrenetEntryFromExternalBuffer(normEntryIndex).xyz;
