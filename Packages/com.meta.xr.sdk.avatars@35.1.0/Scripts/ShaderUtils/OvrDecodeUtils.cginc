...
// 2x 32-bit uint -> 4x 16-bit signed normalized
float4 OvrUnpackSnorm4x16(in ByteAddressBuffer data_buffer, uint address) {
  uint2 packed_data = OvrLoadUint2(data_buffer, address);
  float2 xy = OvrUnpackSnorm2x16(packed_data.x);
  float2 zw = OvrUnpackSnorm2x16(packed_data.y);
